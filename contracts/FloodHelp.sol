...
struct Request {
    ...
    string contact;
    address author;
    uint goal; //em centavos (wei)
    uint balance; 
    bool open;
    uint timestamp; //instante é a quantidade de segundos desde 01/01/1970
}
...
contract FloodHelp {
    ...
    function closeRequest(uint id) public {
        ...
        require(requests[id].open && (msg.sender == author || balance >= goal), 
            unicode"Você não pode fechar este pedido."); // validação
